module CalculatorOfRent
  ...
  class Calculator
    ...
    def logic_calculator
      ...
      if @rent_days <= 6
        if (@start_time_day..@end_time_day).select {|d| (1..5).include?(d.wday)}.size === NUMBER_OF_WORK_DAYS_OF_WEEK
          if @rent_days === NUMBER_OF_WORK_DAYS_OF_WEEK + 1
            @cost = @costworkday + @cost1
          else
            @cost = @costworkday
          end
        elsif (@start_time_day..@end_time_day).select {|d| 6.eql?(d.wday) or 0.eql?(d.wday) }.size === NUMBER_OF_WEEKEND_DAYS
